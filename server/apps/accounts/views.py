...
from .models import (
    ...
    InstructorRequest, UserRole
)
from .serializers import (
    UserSerializer, UserRegistrationSerializer, UserLoginSerializer,
    PasswordChangeSerializer, PasswordResetRequestSerializer,
    PasswordResetConfirmSerializer, InstructorRequestSerializer,
...
class InstructorRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_admin_user:
            return InstructorRequest.objects.select_related(
                'user', 'reviewed_by'
            ).all()
...
class UserManagementViewSet(viewsets.ModelViewSet):
    ...
    def deactivate(self, request, pk=None):
        ...
        if user.is_admin_user and User.objects.filter(role=UserRole.ADMIN, is_active=True).count() <= 1:
            return Response({
                'success': False,
