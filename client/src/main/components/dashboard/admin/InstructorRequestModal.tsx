...
export function InstructorRequestModal({
    request,
    isOpen,
    isReviewing,
    reviewNotes,
    ...
    getStatusColor,
    getStatusBadge
}: InstructorRequestModalProps): JSX.Element | null {
                ...
                >
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-0.5">
                                    ID: <span className="font-mono text-xs">{request.id.slice(0, 8)}...</span>
                                </p>
                            </div>
                            <button
                                onClick={onClose}
                    ...
                    <div className="p-6 overflow-y-auto max-h-[calc(85vh-120px)]">
                        <div className="space-y-6">
                            {/* Applicant Info */}
                            <div className={`p-5 rounded-xl border ${getStatusColor(request.status)}`}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                {request.reviewed_at && (
                                    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                        ...
                                        <p className="text-gray-900 font-medium">
                                            {format(new Date(request.reviewed_at), 'PPP p')}
                                        </p>
                            ...
                            {/* Details Sections */}
                            <DetailSection title="Reason for Application" content={request.reason} />
                            <DetailSection title="Qualifications" content={request.qualifications} />
                            <DetailSection title="Teaching Interests" content={request.teaching_interests} />

                            {request.review_notes && request.review_notes.trim() !== '' && (
                                <DetailSection title="Previous Review Notes" content={request.review_notes} />
                            )}
                            ...
                            {request.status === 'PENDING' && (
                                <div className="pt-6 border-t border-gray-200 space-y-4">
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-3">
                                        <button
                                            onClick={() => onReview(request.id, 'APPROVED', reviewNotes)}
                                            disabled={isReviewing}
                                        ...
                                        <button
                                            onClick={() => onReview(request.id, 'REJECTED', reviewNotes)}
                                            disabled={isReviewing}
