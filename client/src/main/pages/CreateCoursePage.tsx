...
const courseFormInputSchema = z.object({
  ...
  delivery_mode: z.enum(['ONLINE', 'OFFLINE', 'BOTH']),
...
const CreateCoursePage: React.FC = () => {
  ...
  const { addCourse, loading, error } = useCourses();
  ...
  const onSubmit: SubmitHandler<CourseFormInputs> = async (data) => {
    ...
    formData.append('class_starts', parsedData.class_starts || '');
    formData.append('admission_deadline', parsedData.admission_deadline || '');
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-6 px-4 sm:px-6 lg:px-8">
      <SEO title="Create Course" noindex={true} />
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="w-full space-y-6">
              {/* Media Section with FormSidebar side by side */}
              <div className="flex flex-col lg:flex-row gap-6">
                ...
                {/* FormSidebar placed here - side by side with Media Section */}

                <FormSidebar control={control} errors={errors} />
            ...
            {/* Remove the old FormSidebar from here */}
          </div>
        </form>
      </div>
    </div>
