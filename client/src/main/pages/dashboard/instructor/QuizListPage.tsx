...
const QuizListPage: React.FC = () => {
    ...
    const handleOpenEditModal = (quiz: Quiz) => {
        ...
        setFormExpiresAt(quiz.expires_at ? new Date(quiz.expires_at).toISOString().slice(0, 16) : '');
        setIsModalOpen(true);
    };
                            ...
                            <div key={quiz.id} className="bg-white border border-gray-150 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
                                <div>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        {/* Expiry Badge */}
                                        {quiz.expires_at && (
                                            <p className={`text-[10px] mt-1.5 font-semibold ${quiz.is_expired ? 'text-rose-500' : 'text-amber-500'}`}>
                                                {quiz.is_expired
                                                    ? `⛔ Expired: ${new Date(quiz.expires_at).toLocaleString()}`
                                                    : `⏰ Expires: ${new Date(quiz.expires_at).toLocaleString()}`}
                                            </p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="border-t border-gray-100 pt-4 flex items-center justify-between gap-2 mt-auto">
                                    <button
                                        onClick={() => navigate(`/dashboard/instructor/my-courses/${courseId}/quizzes/${quiz.id}/submissions`)}
                                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors cursor-pointer"
                                    >
                                        Submissions
                                    </button>
                                    <button
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-xl border border-gray-100 animate-in fade-in zoom-in-95 duration-150">
                        ...
                        <form onSubmit={handleSaveQuiz} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Link Expiry (optional)</label>
                                <input
                                    type="datetime-local"
                                    value={formExpiresAt}
                                    ...
                                    className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/25 transition-all"
                                />
                                <p className="text-[10px] text-gray-400 mt-1">Leave blank for no expiry. After this date/time the quiz link will be inaccessible.</p>
                            </div>
